...
namespace Xamasoft.JsonClassGenerator.CodeWriters
{
    public class CSharpCodeWriter : ICodeWriter
    {
        ...
        public void WriteFileStart(StringBuilder sw)
        {
            ...
            {
                sw.AppendFormat("{0} class {1}{2}", config.InternalVisibility ? "internal" : "public", config.MainClass);
                sw.AppendLine("{");
            }
        }
        ...
        {   
            sw.AppendLine($"// Data Convertor 로 생성.");
        }
        ...
        public void WriteClass(StringBuilder sw, JsonType type)
        {
            ...
            const string isPartial = "partial";

            // 싱글턴, 상속.
            var className = type.AssignedName;
            string post = config.IsSingleTon ? string.Format(" : Singleton<{0}>", className) : "";
            post = config.IsChildrenClass ? string.Format("{0}, {1}", post, config.Parent) : post;
            ...
            {
                sw.AppendFormat(indentTypes + "{0} {1} class {2}{3}{4}", visibility, isPartial, className, post, Environment.NewLine);
